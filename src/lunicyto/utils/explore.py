...
import typer
from lunicyto.datasets.sipakmed import dataset_info, collect_samples
...
def explore_dataset(
    ...
    typer.echo(f"Classes: {len(info.class_items_count)}")
    typer.echo("\nClass distribution:")

    for cls, cnt in info.class_items_count.items():
    ...
    class_names = list(info.class_items_count.keys())
